...
from greedy_balanced_partition import clustering as greedybp
...
def _sampling(graph, community_type, model):
	...
	if model == "uniform":
		...
		if os.path.exists(community_cache_path):
			...
			elif community_type == 5:
				communities = greedybp(graph)
...
def estimate(graph, adjmat, model, method):
	assert graph.__class__.__name__ == "DiGraph", "Graph isn't digraph"
	assert method in ["baseline1", "baseline2", "baseline3", "new", "weighted", "greedybp"], "Method provided (%s) not exists" % method
	if method == "weighted":
		return _estimate_weighted(graph, adjmat)

	community_type = {"baseline1": 1, "baseline2": 2, "baseline3": 3, "new": 4, "greedybp":5}[method]
